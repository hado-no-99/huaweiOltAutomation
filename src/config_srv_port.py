...
def config_service_port(net_connect):
    ...
    if data != None:
        ...
        service_port_command = net_connect.send_config_set([f"service-port {service_port} vlan {data['vlan']} gpon {data['board']}/{data['card']}/{data['port']} ont {data['ont_id']} gemport {data['gemport_index']} multi-service user-vlan {data['vlan']} tag-transform translate"])
